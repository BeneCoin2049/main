...
(function($) {
    ...
    var flatAccordions = function() {
        ...
        $('.title-wg-category').on('click', function() {
            if (!$(this).parent().is('.active')) {
                $(this).parent().toggleClass('active').children('.content-wg-category').slideToggle(args).children('.content-wg-category').slideToggle(args);
                this.addClass('show');
            } else {
    ...
    var UpImg = function() {
        ...
        function fasterPreview(uploader) {
            if (uploader.files && uploader.files[0]) {
                $('#editimg').attr('src', window.URL.createObjectURL(uploader.files[0]));
            }
        }
